...
const JoinTeam: React.FC = () => {
  ...
  const { register: registerJoin, handleSubmit: handleJoinSubmit } =
  ...
  const onTeamCreate: SubmitHandler<CreateTeamInput> = useCallback(
    async (data) => {
      try {
        setCreateError(undefined);
        refreshTeam(
          (await createTeam(episodeId, data.teamName)) as TeamPrivate,
        );
      } catch (error) {
        setCreateError("Error creating team, try a different name");
      }
    },
    [episodeId],
  );
  const onTeamJoin: SubmitHandler<JoinTeamInput> = useCallback(
    async (data) => {
      try {
