...
func NewGCPTokenedReporter(
	...
	if !onSaturn {
		return &GCPTokenedReporter{nil, userAgent, onSaturn}, nil
	}
	client, err := idtoken.NewClient(ctx, audience)
	if err != nil {
		return nil, fmt.Errorf("idtoken.NewClient: %v", err)
	}
	return &GCPTokenedReporter{client, userAgent, onSaturn}, nil
}
...
func (r *GCPTokenedReporter) Report(ctx context.Context, t *Task) error {
	...
	if r.onSaturn {
		...
		filepath := t.Payload.Metadata.ReportURL
		if err := ioutil.WriteFile(filepath, reqBody, 0644); err != nil {
