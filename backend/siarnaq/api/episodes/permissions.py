...
class IsEpisodeAvailableForSubmission(permissions.BasePermission):
    """
    ...
    def has_permission(self, request, view):
        ...
        return request.method in permissions.SAFE_METHODS or (
            self._allow_frozen or not episode.frozen_submission()
        )


class IsEpisodeAvailableForRegistration(permissions.BasePermission):
    """
    Allows readonly access to visible episodes, and restricts mutation if configured to
    check for frozen episodes. Episodes that are not visible will raise a 404.
    """
    ...
    def has_permission(self, request, view):
        ...
        return not episode.frozen_registration()
