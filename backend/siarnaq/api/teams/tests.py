...
class TeamEligibilityFreezeTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="user", email="user@example.com", email_verified=True
        )
        ...
        # Create an eligibility criterion linked to this episode
        ...
        self.episode.eligibility_criteria.add(self.criterion)
    ...
    def test_not_frozen_if_not_eligible_for_active_tournaments(self):
        now = timezone.now()
        # Create active tournament with eligibility criteria but team not eligible
        ...
        # Add a different criterion so team is not eligible
        other_criterion = EligibilityCriterion.objects.create(
            title="other", description="d", icon="x"
        )
        t.eligibility_includes.add(other_criterion)
...
class SubmissionFreezeBehaviorTestCase(TestCase):
    ...
    def setUp(self):
        self.client = APIClient()
        self.user = User.objects.create_user(
            username="user2", email="u2@example.com", email_verified=True
        )
        self.episode = Episode.objects.create(
            name_short="ep2",
            registration=timezone.now(),
